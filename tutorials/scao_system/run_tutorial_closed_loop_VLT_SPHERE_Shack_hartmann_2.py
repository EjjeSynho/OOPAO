...
from tools.utils import r0, deg2rad, seeing, rad2mas
...
pupil_size = param['resolution'] # real SPHERE case
...
atm = Atmosphere(telescope=tel_vis, \
                 ...
                 altitude=param['altitude'], param=None) #param)
...
for i in range(param['nLoop']):
    ...
    if display == True:
        ...
        if i > 15:
            SE_PSF.append(np.log10(tel_IR_L.PSF_norma_zoom[crop,crop]))
        ...
        cl_plot(list_fig=[atm.OPD, tel_vis.mean_removed_OPD, wfs.cam.frame, dm.coefs, [np.arange(i + 1), residual[:i + 1]],
                          np.log10(tel_IR_L.PSF_norma_zoom[crop,crop]), LE_PSF],
                plt_obj=plot_obj)
        plt.pause(0.1)
